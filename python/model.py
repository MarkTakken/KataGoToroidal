...
class Model:
  ...
  def apply_symmetry(self,tensor,symmetries,xy_shift,inverse):
    ...
    if not inverse:
      if (Space.SETSPACE == Space.TOROIDAL and Space.NETSPACE == Space.PLANAR):
        tensor = tf.roll(tensor, xy_shift, [1,2])
      tensor = tf.cond(
        ud,
  ...
  def build_model(self,config,placeholders):
    ...
    xy_shift = (placeholders["xy_shift"] if "symmetries" in placeholders else
                tf.compat.v1.placeholder(tf.int32, [2], name="xy_shift"))
...
class ModelUtils:
  ...
  def build_model_from_tfrecords_features(features,mode,print_model,trainlog,model_config,pos_len,batch_size,lr_scale=None,gnorm_clip_scale=None,num_gpus_used=1):
    ...
    placeholders["xy_shift"] = tf.random.uniform([2],minval=0,maxval=12,dtype=tf.int32)
